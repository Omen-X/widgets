...
var Select = function () {
  ...
  _createClass(Select, [{
    ...
    value: function parseElement() {
      ...
      var selectedOptionText = selectOptions[0].textContent;

      [].concat(_toConsumableArray(selectOptions)).map(function (option, i) {
    ...
    key: 'renderOption',
    value: function renderOption(element, index) {
    ...
    value: function renderActiveOption() {
      ...
      var activeOption = document.createElement('div');
      activeOption.classList.add(this.selectors.optionActive);
      activeOption.textContent = this.state.selectedOptionText;

      activeOption.addEventListener('click', function (event) {
        event.target.parentNode.classList.toggle(_this.selectors.wrapOpened);
      });

      return activeOption;
    }
  }, {
    key: 'updateActiveOption',
    value: function updateActiveOption() {}
  }, {
    ...
    value: function renderOptionsList() {
      ...
      [].concat(_toConsumableArray(this.state.selectOptions)).map(function (option, i) {
        optionsList.appendChild(_this2.renderOption(option, i));
      });
      ...
      optionsList.addEventListener('click', function (event) {
        ...
        optionsList.parentNode.classList.remove(_this2.selectors.wrapOpened);

        _this2.state.selectedOptionText = text;
        _this2.state.selectedOptionIndex = index;

        optionsList.previousSibling.textContent = text;

        [].concat(_toConsumableArray(_this2.state.selectOptions)).map(function (option, i) {
          if (index === i) option.setAttribute('selected', '');else option.removeAttribute('selected');
        });
      });
...
var element = document.querySelector('#drag-drop');
var items = document.querySelectorAll('#drag-drop li');
...
element.addEventListener('dragstart', function (event) {
  ...
  // clone event.target, hide(opacity) event.target, move copy, drop copy, hide event.target
  console.log(event);
  // event.target.styles.left = 0;
});
